...
#include "robotta_controller/robotta_controller.hpp"
...
using namespace robotta::controller;
...
RobottaDriveController::RobottaDriveController() : controller_interface::ControllerInterface() {}

controller_interface::return_type RobottaDriveController::init(const std::string & controller_name)
{
...
InterfaceConfiguration RobottaDriveController::command_interface_configuration() const
{
...
InterfaceConfiguration RobottaDriveController::state_interface_configuration() const
{
  std::vector<std::string> conf_names;
  for (const auto & joint_name : left_wheel_names_)
  {
...
controller_interface::return_type RobottaDriveController::update()
{
  ...
  for (size_t index = 0; index < wheels.wheels_per_side; ++index)
  {
    registered_left_wheel_handles_[index].velocity.get().set_value(velocity_left_cmd);
    registered_right_wheel_handles_[index].velocity.get().set_value(velocity_right_cmd);
  }

  return controller_interface::return_type::OK;
}

CallbackReturn RobottaDriveController::on_configure(const rclcpp_lifecycle::State &)
{
  auto logger = node_->get_logger();
...
CallbackReturn RobottaDriveController::on_activate(const rclcpp_lifecycle::State &)
{
...
CallbackReturn RobottaDriveController::on_deactivate(const rclcpp_lifecycle::State &)
{
  subscriber_is_active_ = false;
  return CallbackReturn::SUCCESS;
}

CallbackReturn RobottaDriveController::on_cleanup(const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
    return CallbackReturn::ERROR;
...
CallbackReturn RobottaDriveController::on_error(const rclcpp_lifecycle::State &)
{
  if (!reset())
  {
    return CallbackReturn::ERROR;
...
bool RobottaDriveController::reset()
{
...
CallbackReturn RobottaDriveController::on_shutdown(const rclcpp_lifecycle::State &)
{
  return CallbackReturn::SUCCESS;
}

void RobottaDriveController::halt()
{
...
CallbackReturn RobottaDriveController::configure_side(
...
CLASS_LOADER_REGISTER_CLASS(
  robotta::controller::RobottaDriveController, controller_interface::ControllerInterface)